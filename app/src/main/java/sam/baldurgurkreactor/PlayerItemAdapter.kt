...
class PlayerItemAdapter (val viewActions: onPlayerItemSelectedListener, playerItems: MutableList<Item>) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {

    private var items = playerItems as ArrayList<Item>
    interface onPlayerItemSelectedListener {
        fun onPlayerItemSelected(item: Item)
    }
    ...
    inner class ItemViewHolder(parent: ViewGroup) : RecyclerView.ViewHolder(LayoutInflater.from(parent.context).inflate(R.layout.shop_item,parent,false)) {
        fun bind(item: Item) = with(itemView) {
            ...
            super.itemView.setOnClickListener { viewActions.onPlayerItemSelected(item)}
        }
    }
}